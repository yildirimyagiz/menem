    ...
    >
      <div className="relative flex-shrink-0">
        <div className="aspect-w-7 aspect-h-3 md:aspect-h-4 flex h-0 w-full">
          <Image
            fill
            alt={`Background image for ${displayName}`}
            src={bgImage || ""}
        ...
        <div className="absolute inset-x-3 top-3 flex">
          <div
            className="flex items-center justify-center rounded-full bg-neutral-100 px-4 py-1 text-xs font-medium leading-none dark:bg-neutral-800"
            aria-label={`Count: ${convertNumbThousand(count)}`}
          >
...
export default React.memo(CardAuthorBox2);
