...
export function ConversationListItem({
  ...
  return (
    <button
      className={`flex w-full items-center gap-4 rounded-xl px-3 py-2 text-left transition-colors duration-150 focus:outline-none focus:ring-2 focus:ring-primary/50 ${active ? "bg-primary/10 ring-2 ring-primary/30" : "hover:bg-muted/60"}`}
      onClick={onClick}
      tabIndex={0}
      aria-label={`Open conversation with ${user.name}`}
      style={{ boxShadow: active ? "0 2px 8px rgba(0,0,0,0.06)" : undefined }}
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="truncate text-xs text-muted-foreground">
          {lastMessage ?? t('sidebar.noMessages', { defaultValue: 'No messages yet.' })}
        </div>
      </div>
      {unreadCount > 0 && (
...
export function ChatSidebar({
  ...
  return (
    <aside
      className={`fixed left-0 top-0 z-30 flex h-full max-w-xs flex-col rounded-r-2xl border-r bg-white shadow-xl transition-transform duration-300 ease-in-out md:static md:z-auto md:block md:h-auto md:translate-x-0 md:rounded-none md:shadow-none ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
    ...
    >
      <div className="flex items-center justify-between border-b bg-white px-6 py-4 text-xl font-bold tracking-tight md:justify-start">
        <span className="text-lg font-bold">{t('sidebar.title', { defaultValue: 'Conversations' })}</span>
        <button
          className="ml-auto rounded-full bg-slate-200 p-1.5 text-gray-700 hover:bg-slate-300 focus:outline-none md:hidden"
          aria-label={t('sidebar.closeSidebar', { defaultValue: 'Close conversations sidebar' })}
          type="button"
