...
import { TaskLabelEnum } from "@reservatior/validators";

import ButtonPrimary from "~/shared/ButtonPrimary";
import ButtonThird from "~/shared/ButtonThird";
import Checkbox from "~/shared/Checkbox";

const labelOptions = TaskLabelEnum.options.map((name) => ({ name }));

const RenderTabsLabel: React.FC = () => {
  const t = useTranslations("tasks");
  const [selected, setSelected] = React.useState<string | null>(null);
  const [search, setSearch] = React.useState("");
  const filteredLabelOptions = labelOptions.filter((item) =>
    t("label." + item.name)
      .toLowerCase()
        ...
        <>
          ...
          >
            <span className="inline-flex items-center">
              <i className="las la-tags mr-2 text-lg"></i>
              {t("label.title")}
            </span>
            <i
                    ...
                    filteredLabelOptions.map((item) => (
                      <button
                        ...
                        className={`min-w-0 max-w-full truncate rounded-full px-4 py-2 text-sm font-medium shadow-sm transition-all focus:outline-none focus:ring-2 focus:ring-blue-400 ${
                          selected === item.name
                            ? "bg-gradient-to-r from-blue-600 to-purple-600 font-semibold text-white shadow-lg"
                            : "bg-blue-50 font-normal text-blue-700 hover:bg-blue-100"
                        } `}
                        onClick={() => {
...
export default RenderTabsLabel;
