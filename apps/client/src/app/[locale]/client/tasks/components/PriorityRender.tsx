...
import { TaskPriorityEnum } from "@reservatior/validators";
...
interface PriorityRenderProps {
  ...
  renderXClear: () => React.ReactNode;
}

// Human-friendly priority labels and descriptions
const priorityMeta: Record<string, { label: string; description: string }> = {
  LOW: { label: "Low", description: "Low priority tasks" },
  MEDIUM: { label: "Medium", description: "Medium priority tasks" },
  HIGH: { label: "High", description: "High priority tasks" },
  URGENT: {
    label: "Urgent",
...
const priorityOptions = TaskPriorityEnum.options.map((name) => ({
  name,
}));

const PriorityRender: React.FC<PriorityRenderProps> = ({
  isRecurring,
  setIsRecurring,
  renderXClear,
}) => {
  ...
  const filteredPriorityOptions = priorityOptions.filter((item) =>
        ...
        <>
          ...
          >
            <span className="inline-flex items-center">
              <i className="las la-flag mr-2 text-lg"></i>
              {t("priority.title")}
            </span>
            <i
              ...
              <div className="overflow-hidden rounded-3xl border-0 bg-white/60 shadow-2xl ring-1 ring-blue-100 backdrop-blur-lg">
                ...
                <div className="scrollbar-thin scrollbar-thumb-blue-200 scrollbar-track-transparent flex max-h-52 flex-wrap gap-2 overflow-y-auto px-6 pb-4">
                  {filteredPriorityOptions.length === 0 ? (
                    ...
                    filteredPriorityOptions.map((item) => (
                      <button
                        key={item.name}
