...
// Agency Statistics Component
const AgencyStats = ({ stats }: { stats: any }) => {
...
// Enhanced Agency Card Component
const AgencyCard = React.memo<{
  ...
  ({
    ...
    const handleViewProfile = useCallback(() => {
      ...
      console.log(`Viewing agency profile: ${agency.id}`);
    }, [agency.id]);
    ...
    return (
      <Card className="agency-listing-card group hover:shadow-xl transition-all duration-300 hover:scale-[1.02]">
        <CardContent className="p-6">
          <div className="mb-4 flex items-start space-x-4">
            <Avatar
              sizeClass="h-14 w-14 text-lg"
              imgUrl={agency.logoUrl ?? ""}
              hasChecked={agency.isActive}
              hasCheckedClass="w-6 h-6 -top-1 right-1"
            />
            <div className="flex-1 min-w-0">
              <h3 className="text-xl font-semibold text-gray-900 dark:text-white truncate">
                {agency.name ?? t('list.agency')}
              </h3>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <Badge variant={agency.isActive ? "default" : "secondary"}>
                  {agency.status?.replace("_", " ") || t('list.agency')}
                </Badge>
                {subscription && (
                  <>
          ...
          <div className="mb-4 grid grid-cols-3 gap-4 text-center">
            <div className="agency-stat-item">
              <Building2 className="agency-stat-icon" />
              <span className="agency-stat-value">{agency._count?.Property || 0}</span>
              <span className="agency-stat-label">{t('list.properties')}</span>
            </div>
            <div className="agency-stat-item">
              <Users className="agency-stat-icon" />
              <span className="agency-stat-value">{agency._count?.Agent || 0}</span>
              <span className="agency-stat-label">{t('list.agents')}</span>
            </div>
            <div className="agency-stat-item">
              <Activity className="agency-stat-icon" />
              <span className="agency-stat-value">{agency._count?.User || 0}</span>
              <span className="agency-stat-label">{t('list.users')}</span>
            </div>
          </div>
          ...
          <div className="flex flex-col space-y-2">
            <Link
              href={`/client/agencies/${agency.id}`}
              className="agency-card-action"
              onClick={handleViewProfile}
            >
              <span>{t('list.viewProfile')}</span>
              <ArrowRightIcon className="agency-card-action-icon" />
            </Link>
...
const EmptyState = ({ t }: { t: ReturnType<typeof useTranslations> }) => {
  return (
    <Card className="agency-empty-state">
...
const ErrorComponent = ({ t }: { t: ReturnType<typeof useTranslations> }) => {
  return (
    <Card className="agency-error-state">
      <CardContent className="p-12 text-center">
...
const APIStatusComponent = ({ 
  ...
  const apiStatuses = [
    {
      name: "Agencies API",
      status: agenciesResponse ? "✅ " + t('list.apiStatus.connected') : "❌ " + t('list.apiStatus.disconnected'),
      data: agenciesResponse?.data?.data?.length || 0,
      label: "agencies"
    },
    {
      name: "Statistics API", 
      status: agencyStats ? "✅ " + t('list.apiStatus.connected') : "❌ " + t('list.apiStatus.disconnected'),
      data: agencyStats?.total || 0,
      label: "total agencies"
    },
    {
      name: "Subscription Packages API",
      status: subscriptionPackages ? "✅ " + t('list.apiStatus.connected') : "❌ " + t('list.apiStatus.disconnected'), 
      data: subscriptionPackages?.length || 0,
      label: "packages"
    },
    {
      name: "Subscriptions API",
      status: subscriptionsResponse ? "✅ " + t('list.apiStatus.connected') : "❌ " + t('list.apiStatus.disconnected'),
      data: subscriptionsResponse?.data?.data?.length || 0,
      label: "subscriptions"
    },
    {
      name: "Featured Properties API",
      status: featuredProperties ? "✅ " + t('list.apiStatus.connected') : "❌ " + t('list.apiStatus.disconnected'),
...
const AgenciesPage = () => {
  ...
  // Fetch agencies with caching and stale-while-revalidate
  const {
    data: agenciesResponse,
    isLoading,
    isError,
    refetch: refetchAgencies,
  } = api.agency.public.useQuery(
    {
      page: currentPage,
      pageSize,
  ...
  const { data: agencyStats } = api.agency.stats.useQuery(
    undefined,
    {
      refetchOnWindowFocus: false,
      refetchOnMount: false,
  ...
  // Fetch featured properties for agencies
  ...
  // Fetch analytics data for agencies (if available)
  ...
  // Memoized agencies data
  const agencies = useMemo(() => {
    const arr = (agenciesResponse as any)?.data?.data;
    return Array.isArray(arr) ? arr : [];
  }, [agenciesResponse]);

  // Memoized pagination data
  const paginationData = useMemo(() => {
    const response = agenciesResponse as any;
    return {
  ...
  }, [agenciesResponse, pageSize]);

  // Memoized agency IDs for subscription query
  const agencyIds = useMemo(() => {
    return agencies.map((agency: any) => agency.id);
  }, [agencies]);

  // Fetch subscriptions for all agencies with conditional query
  ...
  const getAgencySubscription = useCallback(
    (agencyId: string) => {
      return subscriptions.find(
        (sub: any) =>
          sub.entityId === agencyId &&
  ...
  const getStatusBadge = useCallback((status: string) => {
    switch (status) {
      case "ACTIVE":
        return (
          <Badge className="bg-green-100 text-xs text-green-800">{t('list.subscription.active')}</Badge>
        );
      case "TRIAL":
        return (
          <Badge className="bg-blue-100 text-xs text-blue-800">{t('list.subscription.trial')}</Badge>
        );
      case "EXPIRED":
        return (
          <Badge className="bg-red-100 text-xs text-red-800">{t('list.subscription.expired')}</Badge>
        );
      case "SUSPENDED":
        return (
          <Badge className="bg-yellow-100 text-xs text-yellow-800">
            {t('list.subscription.suspended')}
          </Badge>
        );
      default:
        return (
  ...
  const getTierBadge = useCallback((tier: string) => {
    switch (tier) {
      case "BASIC":
        return <Badge className="bg-gray-100 text-xs text-gray-800">{t('list.tier.basic')}</Badge>;
      case "PRO":
        return (
          <Badge className="bg-purple-100 text-xs text-purple-800">{t('list.tier.pro')}</Badge>
        );
      case "ENTERPRISE":
        return (
          <Badge className="bg-orange-100 text-xs text-orange-800">
            {t('list.tier.enterprise')}
          </Badge>
        );
      default:
        return <Badge className="bg-gray-100 text-xs text-gray-800">{tier}</Badge>;
    }
  }, [t]);

  // Memoized agency cards
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <Card className="agency-filter-card mb-6">
        <CardContent className="p-4">
