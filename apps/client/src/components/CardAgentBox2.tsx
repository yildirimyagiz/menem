...
interface CardAgentBox2Props {
  className?: string;
  agent: any; // Change to TAgent if available
}

const CardAgentBox2: FC<CardAgentBox2Props> = ({ className = "", agent }) => {
  const {
    displayName,
    handle = "/",
    avatarUrl,
  ...
  // Fallback image (local or data URL)
  const fallbackBgImage = "/images/agent-bg-placeholder.jpg";
  const imageSrc =
    bgImage && typeof bgImage === "string" && bgImage.trim() !== ""
      ? bgImage
      : fallbackBgImage;
  return (
    <Link
      ...
      <div className="relative shrink-0">
        ...
        <div className="absolute inset-x-3 top-3 flex">
          <div className="flex items-center justify-center rounded-full bg-neutral-100 px-4 py-1 text-xs font-medium leading-none dark:bg-neutral-800">
            ...
            <ArrowRightIcon className="ms-3 size-4 text-yellow-600 rtl:rotate-180" />
          </div>
        </div>
      </div>
