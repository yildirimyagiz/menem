...
async function main() {
      ...
      try {
        ...
        return await db.agent.create({
          data: {
            ...
            ) as AgentSpecialities[], // Add type assertion if TS complains after faker update
            settings: {
      ...
      try {
        return await db.provider.create({
          data: {
            ...
            isActive: faker.datatype.boolean(), // Corrected: faker.datatype.boolean() takes 0-1 arguments
            commission: Number(
              faker.finance.amount({ min: 1, max: 25, dec: 2 }),
            ), // Corrected: faker.finance.amount expects an options object
            createdAt: faker.date.past(),
            updatedAt: new Date(),
      ...
      try {
        ...
        return await db.property.create({
          data: {
            ...
            extraChargeId: getRandomElement(extraChargesSeeded)?.id, // Corrected: faker.finance.amount expects an options object
            marketValue: Number(
              faker.finance.amount({ min: 50000, max: 5000000, dec: 2 }),
            ), // Corrected: faker.finance.amount expects an options object
            taxValue: Number(
              faker.finance.amount({ min: 40000, max: 4000000, dec: 2 }),
            ), // Corrected: faker.finance.amount expects an options object
            insuranceValue: Number(
              faker.finance.amount({ min: 0, max: 1000000, dec: 2 }),
            ),
  ...
  const photosSeeded = await Promise.all(
    Array.from({ length: 90 }).map(async () => {
      // Adjusted for more photos, e.g., 3 per property/post if 30 properties/posts
      try {
        ...
        return await db.photo.create({
          data: {
            ...
            featured: faker.datatype.boolean(), // Corrected
            width: faker.number.int({ min: 600, max: 1920 }),
            height: faker.number.int({ min: 400, max: 1080 }),
      ...
      try {
        ...
        return await db.task.create({
          data: {
            ...
            extraChargeId: extraCharge?.id, // Corrected: Ensure dueDate is Date | null
      ...
      try {
        ...
        return await db.pricingRule.create({
          data: {
            ...
            currencyId: currency.id,
            weekdayPrices: {
              mon: Number(faker.finance.amount({ min: 50, max: 150, dec: 0 })), // Corrected: faker.finance.amount expects an options object
              tue: Number(faker.finance.amount({ min: 50, max: 150, dec: 0 })),
            },
            taxRules: { vat: 0.2, cityTax: 5.0 }, // Corrected: ensure this is valid JSON or handle as per schema
  ...
  const sessionsSeeded = await Promise.all(
    Array.from({ length: 30 }).map(async () => {
      try {
        ...
        console.error("Error creating session:", e); // Corrected: removed extra comma
        return null;
      }
    }),
  ).then(
    ...
    Array.from({ length: 30 }).map(async () => {
      try {
        ...
        return await db.mortgage.create({
          data: {
            propertyId: property.id, // Corrected: faker.finance.amount expects an options object
            lender: faker.company.name(), // Corrected: faker.finance.amount expects an options object
            principal: Number(
              faker.finance.amount({ min: 50000, max: 2000000, dec: 2 }),
            ),
            interestRate: faker.number.float({
              min: 1,
              max: 8,
              multipleOf: 0.01, // Corrected: precision to multipleOf
            }),
            startDate: faker.date.past(),
        ...
        return await db.reservation.create({
          data: {
            ...
            checkInTime: faker.datatype.boolean()
              ? faker.date.soon() // Corrected: ensure this is a Date
              : undefined,
      ...
      try {
        ...
        return await db.taxRecord.create({
          data: {
            propertyId: property.id,
            year:
              new Date().getFullYear() - faker.number.int({ min: 0, max: 3 }), // Corrected: faker.finance.amount expects an options object
            amount: Number(
      ...
      try {
        ...
        return await db.event.create({
          data: {
            ...
            createdById: createdBy.id,
            isActive: faker.datatype.boolean(),
            createdAt: faker.date.past(), // Corrected: ensure this is a Date
            updatedAt: new Date(),
            attendees: {
              // Added attendees
      ...
      try {
        ...
        return await db.offer.create({
          data: {
            ...
            }), // Corrected: ensure this is a Date
        ...
        return await db.discount.create({
          data: {
            ...
            code: faker.datatype.boolean()
              ...
              : undefined,
            value: faker.number.float({ min: 5, max: 50, multipleOf: 0.01 }), // Corrected: precision to multipleOf
            ...
            isActive: faker.datatype.boolean(),
            createdAt: faker.date.past(),
            updatedAt: new Date(), // Corrected: ensure this is a Date
            propertyId: property.id,
            pricingRuleId: pricingRule?.id,
          },
        });
      ...
      try {
        ...
        return await db.analytics.create({
          data: {
            ...
            taskId: task?.id, // Corrected: ensure this is a string or undefined
          },
        });
      } catch (e) {
  ...
  const availabilitiesSeeded = await Promise.all(
    Array.from({ length: 30 }).map(async () => {
      // Define property and date here so they are in scope for the catch block if needed for logging
      let currentProperty: (typeof propertiesSeeded)[0] | undefined;
      let currentDate: Date | undefined;
      try {
        currentProperty = getRandomElement(propertiesSeeded);
        if (!currentProperty) return null;
        currentDate = faker.date.soon({ days: 90 });
        const pricingRule = getRandomElement(
          pricingRulesSeeded.filter(
            (pr) => pr?.propertyId === currentProperty.id,
          ),
        );
        const reservationForDate = getRandomElement(
          reservationsSeeded.filter(
            (r) =>
              r?.propertyId === currentProperty.id &&
              r.startDate <= currentDate! && // Use currentDate
              r.endDate >= currentDate!, // Use currentDate
          ),
        );
        ...
        return await db.availability.create({
          data: {
            ...
            pricingRuleId: pricingRule?.id,
            totalUnits: currentProperty.bedrooms || 1,
            availableUnits: reservationForDate
              ? 0
              : currentProperty.bedrooms || 1,
            bookedUnits: reservationForDate ? currentProperty.bedrooms || 1 : 0,
            blockedUnits: 0,
            basePrice:
              pricingRule?.basePrice ||
              Number(faker.finance.amount({ min: 50, max: 500, dec: 2 })),
            // Corrected: faker.finance.amount expects an options object
            currentPrice:
              pricingRule?.basePrice || // Corrected: faker.finance.amount expects an options object
              Number(faker.finance.amount({ min: 50, max: 500, dec: 2 })),
            minNights: pricingRule?.minNights || 1,
            maxNights: pricingRule?.maxNights || 30,
            maxGuests: (currentProperty.bedrooms || 1) * 2,
            createdAt: faker.date.past(),
            updatedAt: new Date(),
          },
        });
        ...
        if (e.code === "P2002") {
          // Handle unique constraint (propertyId, date)
          console.warn(
            `Skipping availability creation due to unique constraint for property ${currentProperty?.id} on date ${currentDate?.toISOString().split("T")[0]}.`,
          );
          return null;
        }
        ...
        return await db.ticket.create({
          data: {
            ...
            closedAt: faker.datatype.boolean()
              ? faker.date.recent() // Corrected: ensure this is a Date
              : undefined,
          },
        });
      } catch (e) {
        ...
        return await db.commissionRule.create({
          data: {
            ...
            commission: faker.number.float({
              min: 0.01,
              max: 0.25,
              multipleOf: 0.001, // Corrected: precision to multipleOf
            }),
            minVolume: faker.datatype.boolean()
              ? faker.number.int({ min: 1000, max: 10000 })
              : undefined,
            maxVolume: faker.datatype.boolean()
              ? faker.number.int({ min: 10000, max: 100000 }) // Corrected: ensure this is a number or undefined
              : undefined,
            conditions: {
    ...
    if (selectedHashtags.length > 0) {
      try {
        await db.post.update({
          where: { id: post.id }, // Corrected: A post has one hashtagId
          data: { hashtagId: getRandomElement(selectedHashtags)?.id },
        });
      } catch (e) {
  ...
  const increasesSeeded = await Promise.all(
    Array.from({ length: 30 }).map(async () => {
      try {
        ...
        const oldRent = Number(
          faker.finance.amount({ min: 500, max: 3000, dec: 2 }),
        );
        return await db.increase.create({
          ...
          data: {
            ...
            newRent:
              oldRent *
              faker.number.float({ min: 1.05, max: 1.2, multipleOf: 0.01 }), // Corrected: precision to multipleOf
            effectiveDate: faker.date.future(), // Corrected: ensure this is a Date
    ...
    if (faker.datatype.boolean(0.2)) {
      const increase = getRandomElement(
        increasesSeeded.filter(
          (i) => i && !i.Offer && i.propertyId === offer.propertyId, // Assuming 'Offer' is the correct relation name as per schema
        ),
      );
      if (increase) {
        try {
          ...
          await db.increase.update({
            ...
            data: { Offer: { connect: { id: offer.id } } }, // Corrected: Property 'offer' does not exist. Did you mean 'Offer'?
          });
        } catch (e) {
          console.error(
      ...
      try {
        ...
        return await db.expense.create({
          data: {
            ...
            tenantId: tenant?.id,
            agencyId: agency?.id,
            type: faker.helpers.arrayElement(Object.values(ExpenseType)), // Corrected: faker.finance.amount expects an options object
            amount: Number(
              faker.finance.amount({ min: 10, max: 1000, dec: 2 }),
            ),
            currencyId: currency.id, // Corrected: faker.finance.amount expects an options object
            dueDate: faker.datatype.boolean() ? faker.date.future() : undefined,
            paidDate: faker.datatype.boolean() // Corrected: ensure this is a Date or undefined
              ? faker.date.recent()
              : undefined,
  ...
  // --- Seed MLConfiguration ---
  try {
    await db.mLConfiguration.upsert({
      // Corrected: mLCConfiguration to mLConfiguration
      where: { id: "singleton" },
      update: {
      ...
      create: {
        id: "singleton",
        enableAutoTagging: faker.datatype.boolean(0.8), // Default to true more often
        qualityThreshold: faker.number.float({
          min: 0.6,
          max: 0.85,
          multipleOf: 0.01, // Corrected: precision to multipleOf
    ...
    console.log(
      `Upserted MLConfiguration. Total MLConfigurations in DB: ${await db.mLConfiguration.count()}`, // Corrected: mLCConfiguration to mLConfiguration
    );
  } catch (e) {
    console.error("Error upserting MLConfiguration:", e);
  }
